    ...
    <div className="container-wide py-24 sm:py-32">
      <div className="flex flex-col gap-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <h1 className="apple-display text-[var(--color-text)]">Your Loot History</h1>
          <button 
           onClick={() => navigate('/')}
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            {displayOrders.length > 0 ? displayOrders.map((order) => (
              <div key={order._id} className="glass-panel p-6 rounded-2xl flex flex-col gap-4">
                <div className="flex justify-between items-start border-b border-[var(--color-glass-border)] pb-4">
                  <div>
                    <p className="text-[12px] text-[var(--color-text-muted)] uppercase tracking-wider">Order ID</p>
                    <p className="font-mono text-[14px] text-[var(--color-text)]">#{order._id.toUpperCase()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[12px] text-[var(--color-text-muted)] uppercase tracking-wider">Status</p>
              ...
              <div className="text-center py-20 glass-panel rounded-2xl">
                <p className="text-[var(--color-text-muted)]">Nothing here yet. Go treasure hunting in the homepage!</p>
              </div>
            )}
          </div>
        )}
