...
import { HiShieldCheck, HiPencil, HiTrash, HiPlus, HiX, HiExclamation, HiUserCircle } from 'react-icons/hi'
...
export default function AdminDashboard() {
  ...
  const [formData, setFormData] = useState({
    id: '', brand: '', name: '',
    category: '', image: '', description: '', price: 0,
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const url = showEditModal ? `${API_URL}/products/${selectedProduct.id}` : `${API_URL}/products`
      ...
      if (res.ok) {
        setMessage(showEditModal ? 'Product updated!' : 'New card added to the vault!')
        fetchData()
        closeModals()
        setTimeout(() => setMessage(''), 3000)
      } else {
  ...
  const openEdit = (product) => {
    ...
    setFormData({
      ...
      stock: product.stock, popular: product.popular
    })
      ...
      <div className="flex flex-col gap-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <h1 className="apple-display text-[var(--color-text)]">Vault Command Center</h1>
          </div>
          <button 
           onClick={() => navigate('/')}
                  ...
                  <tbody className="divide-y divide-[var(--color-glass-border)]">
                    {products.map((p) => (
                      <tr key={p.id} className="hover:bg-white/5 transition group">
                        <td className="px-6 py-4 font-mono text-[var(--color-text-muted)]">{p.id}</td>
                        <td className="px-6 py-4">
                           <div className="flex items-center gap-3">
                             <img src={p.image} className="h-8 w-8 rounded-md object-cover" alt="" />
                        ...
                        <td className="px-6 py-4">
                           <div className="flex items-center gap-3">
                             ...
                             >
                               {[0, 1, 2, 3, 4, 5].map(n => (
                                 <option key={n} value={n} className="bg-[var(--color-background)]">{n === 0 ? 'Out of Stock (0)' : n}</option>
                               ))}
                             </select>
                             {p.stock <= 2 && p.stock > 0 && (
                               <span className="text-[11px] font-bold text-red-500 animate-pulse">LOW</span>
                             )}
                           </div>
                        </td>
                    ...
                    {products.length === 0 && (
                      <tr><td colSpan="4" className="px-6 py-20 text-center text-[var(--color-text-muted)]">The vault is lonely. Add some cards!</td></tr>
                    )}
                  </tbody>
                </table>
              )}
                  ...
                  <tbody className="divide-y divide-[var(--color-glass-border)]">
                    {users.map((u) => (
                      <tr key={u._id} className="hover:bg-white/5 transition">
                        ...
                        <td className="px-6 py-4">
                           <span className={`px-2 py-0.5 rounded-full text-[12px] ${u.role === 'admin' ? 'bg-purple-500/20 text-purple-400' : 'bg-green-500/20 text-green-400'}`}>
                             {u.role}
                           </span>
                        </td>
                        <td className="px-6 py-4 text-right space-x-3">
                          ...
                          <button 
                            onClick={() => { setSelectedUser(u); setShowUserDeleteModal(true); }}
                            ...
                            title={u.email === 'admin@cardvault.com' ? 'Master Admin Locked' : 'Delete User'}
                          >
                            <HiTrash className="h-4 w-4" />
                          </button>
                        </td>
                  ...
                  <tbody className="divide-y divide-[var(--color-glass-border)]">
                    {orders.map((o) => (
                      <tr key={o._id} className="hover:bg-white/5 transition">
                        ...
                        <td className="px-6 py-4 text-[var(--color-text-muted)] font-medium underline">{o.user?.name || 'Guest'}</td>
                        <td className="px-6 py-4">{o.orderItems.length} items</td>
          ...
          <div className="glass-strong w-full max-w-2xl rounded-[24px] overflow-hidden animate-scale-in">
            ...
            <form onSubmit={handleFormSubmit} className="p-6">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-[60vh] overflow-y-auto px-1">
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Product ID (Unique)</label>
                    <input 
                      type="text" required
                      value={formData.id}
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Brand Name</label>
                    <input 
                      type="text" required
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Full Product Name</label>
                    <input 
                      type="text" required
                      value={formData.name}
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Category</label>
                    <input 
                      type="text" required
                  ...
                  <div className="sm:col-span-2">
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Image URL</label>
                    <input 
                      type="url" required
                  ...
                  <div className="sm:col-span-2">
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Description</label>
                    <textarea 
                      rows="2"
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Selling Price (Number)</label>
                    <input 
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Validity Date (YYYY-MM-DD)</label>
                    <input 
                      type="date"
                      value={formData.validityEndDateTime}
                      onChange={e => setFormData({...formData, validityEndDateTime: e.target.value})}
                      className="glass-input w-full mt-1 rounded-xl px-4 py-2"
                    />
                  </div>
                  <div>
                    <label className="text-[12px] font-medium text-[var(--color-text-muted)]">Initial Stock</label>
                    <input 
                  ...
                  <div className="flex items-center gap-3 mt-6">
                    ...
                    />
                    <label htmlFor="popular" className="text-[14px]">Mark as Popular?</label>
                  </div>
               </div>
               <div className="mt-8 flex gap-4">
                  ...
                  >
                    {formLoading ? 'Working Magic...' : showEditModal ? 'Update Product' : 'Add Product'}
                  </button>
               </div>
            </form>
          </div>
      ...
      {showUserEditModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass-strong w-full max-w-md rounded-[24px] overflow-hidden animate-scale-in">
            <div className="flex items-center justify-between border-b border-[var(--color-glass-border)] px-6 py-4 bg-white/5">
               <h2 className="text-xl font-bold text-[var(--color-text)]">Edit Vault Member</h2>
               <button onClick={closeModals} className="p-2 rounded-full hover:bg-white/10 transition">
                 <HiX className="h-5 w-5" />
               </button>
            </div>
            <form onSubmit={handleUserFormSubmit} className="p-6 space-y-5">
               <div>
                 <label className="text-[12px] font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Member Name</label>
                 <input 
                   type="text" required
               ...
               <div>
                  <label className="text-[12px] font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Assigned Role</label>
                  <select 
                  ...
                  >
                    <option value="user" className="bg-[var(--color-background)]">User</option>
                    <option value="admin" className="bg-[var(--color-background)]">Admin</option>
                  </select>
      ...
      {showUserDeleteModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="glass-strong w-full max-w-sm rounded-[32px] p-8 text-center animate-scale-in border-red-500/30">
             ...
             <h3 className="text-xl font-bold text-[var(--color-text)]">Revoke Access?</h3>
             <p className="mt-4 text-[15px] text-[var(--color-text-muted)] leading-relaxed">
               You are about to remove <span className="font-bold text-[var(--color-text)]">"{selectedUser?.name}"</span> from the vault. They will lose all access immediately.
             </p>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="glass-strong w-full max-w-sm rounded-[32px] p-8 text-center animate-scale-in border-red-500/30">
             ...
             <h3 className="text-xl font-bold text-[var(--color-text)]">Burn the Bridge?</h3>
             <p className="mt-4 text-[15px] text-[var(--color-text-muted)] leading-relaxed">
               You are about to delete <span className="font-bold text-[var(--color-text)]">"{selectedProduct?.name}"</span>. This action is final and can&apos;t be undone. Are you sure you want to proceed?
             </p>
             <div className="mt-8 flex flex-col gap-3">
                <button 
